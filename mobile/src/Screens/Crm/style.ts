import {Platform, StyleSheet} from 'react-native';
import {Colors, rgba} from '../../Theme/Colors';
import {hp, rfs, wp} from '../../Theme/Responsiveness';
import Typography from '../../Theme/Typography';

export const styles = StyleSheet.create({
  mainView: {
    backgroundColor: 'white',
    flex: 1,
  },
  subContainer: {
    paddingHorizontal: wp(3),
    flex: 1,
  },
  rowContainer: {
    flexDirection: 'row',
    alignItems: 'center',
    flexWrap:'wrap'
  },
  carPropContainer: {
    flexDirection: 'row',
    alignItems: 'center',
    marginTop: hp(1),
  },
  carModelContainer: {
    flexDirection: 'row',
    alignItems: 'center',
    marginVertical: hp(1),
  },
  priceTxt: {
    color: 'white',
    fontSize: wp(3.5),
    fontFamily: Typography?.poppins?.Regular,
  },
  carName: {
    color: Colors.black,
    fontSize: wp(3.7),
    fontFamily: Typography?.poppins?.SemiBold,
  },
  optionName: {
    color: Colors.black,
    fontSize: wp(3.5),
    fontFamily: Typography?.poppins?.Regular,
  },
  info: {
    color: Colors.greyIcn,
    fontSize: wp(3.3),
    fontFamily: Typography?.poppins?.Regular,
  },
  itemContainer: {
    backgroundColor: Colors.dullWhite,
    paddingHorizontal: wp(3),
    paddingVertical: hp(2),
    borderRadius: wp(3),
    marginBottom: hp(2),
  },
  modalContainer: {
    backgroundColor: 'white',
    paddingHorizontal: wp(3),
    paddingVertical: hp(2),
    borderRadius: wp(5),
    width: wp(90),
    alignItems: 'center',
  },
  rowSpaceContainer: {
    flexDirection: 'row',
    alignItems: 'flex-start',
    justifyContent: 'space-between',
  },
  optionValueContainer: {
    flexDirection: 'row',
    alignItems: 'flex-start',
    justifyContent: 'space-between',
    marginTop: hp(1),
  },
  optionBackground: {
    backgroundColor: Colors.dullWhite,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    paddingHorizontal: wp(3),
    paddingVertical: hp(0.6),
    width: wp(40),
    borderRadius: wp(2),
    marginBottom: hp(2),
  },
  filterButtonsContainer: {
    flexDirection: 'row',
    marginTop: hp(2),
  },
  optionSpaceContainer: {
    flexDirection: 'row',
    alignItems: 'flex-start',
    justifyContent: 'space-between',
    marginBottom: hp(0.5),
  },
  searchContainer: {
    marginTop: hp(2.5),
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  },
  placeholderContainer: {
    marginVertical: hp(1),
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  },
  iconStyle: {
    width: wp(12),
    height: hp(6),
  },
  backIcn: {
    width: wp(4),
    height: hp(2),
  },
  vehicleIcn: {
    width: wp(6),
    height: hp(3),
    marginRight: wp(2),
  },
  img: {
    width: wp(80),
    height: hp(20),
    alignSelf: 'center',
  },
  carImg: {
    width: wp(18),
    height: hp(7),
  },
  listForwardIcn: {
    width: wp(3),
    height: hp(1.5),
    marginLeft: wp(0.5),
  },
  bottomIcn: {
    width: wp(8),
    height: hp(3.5),
  },
  crossIcn: {
    width: wp(6),
    height: hp(3),
  },
  gmailIcn: {
    width: wp(4.5),
    height: hp(2.3),
    marginRight: wp(2),
  },
  separator: {
    width: wp(5),
    height: hp(2.2),
    marginHorizontal: wp(4),
  },
  line: {
    backgroundColor: rgba(Colors.greyLine, 0.2),
    height: hp(0.08),
    // width: wp(88),
    marginVertical: hp(2),
  },
  option: {
    color: Colors.greyIcn,
    fontFamily: Typography?.poppins?.Regular,
    fontSize: wp(3.2),
  },
  value: {
    color: Colors.black,
    fontFamily: Typography?.poppins?.Medium,
    fontSize: wp(3.5),
  },
  orangeValue: {
    color: Colors.lightOrange,
    fontFamily: Typography?.poppins?.Medium,
    fontSize: wp(3.5),
  },
  subValue: {
    color: Colors.red,
    fontFamily: Typography?.poppins?.Medium,
    fontSize: wp(3),
  },
  spamValue: {
    color: Colors.black,
    fontFamily: Typography?.poppins?.Regular,
    textDecorationLine: 'underline',
    fontSize: wp(3),
  },
  confirmationText: {
    color: Colors.greyIcn,
    textAlign: 'center',
    width: wp(58),
    marginTop: hp(0.6),
    fontFamily: Typography?.poppins?.Regular,
    fontSize: wp(3.3),
  },
  modalHeading: {
    marginTop: hp(1),
    color: Colors.black,
    fontFamily: Typography?.poppins?.Medium,
    fontSize: wp(4),
  },
  blackDot: {
    paddingHorizontal: wp(0.7),
    paddingVertical: wp(0.7),
    borderRadius: wp(100),
    backgroundColor: Colors.black,
    marginHorizontal: wp(1),
  },
  phoneIcn: {
    width: wp(4.2),
    height: hp(2.1),
  },
  printerIcn: {
    width: wp(5),
    height: hp(2.5),
  },
  optionIcn: {
    width: wp(5.5),
    height: hp(3),
    marginTop: hp(0.2),
  },
  searchBar: {width: wp(78), marginTop: 0},
  priceContainer: {
    backgroundColor: Colors.primary,
    paddingHorizontal: wp(2.7),
    paddingVertical: hp(0.6),
    borderRadius: wp(5),
  },
  carInfoContainer: {
    marginLeft: hp(1.5),
  },
  description: {
    color: Colors.greyIcn,
    fontFamily: Typography?.poppins?.Regular,
    fontSize: wp(3.1),
  },
  detailText: {
    color: Colors.black,
    fontFamily: Typography?.poppins?.Regular,
    fontSize: wp(3.3),
  },
  leadText: {
    color: Colors.black,
    fontFamily: Typography?.poppins?.Medium,
    fontSize: wp(4.5),
  },
  itemName: {
    color: Colors.black,
    fontFamily: Typography?.poppins?.SemiBold,
    fontSize: wp(4.5),
  },
  nextImg: {width: wp(4), height: hp(2)},
  modelText: {
    color: Colors.black,
    fontFamily: Typography?.poppins?.SemiBold,
    fontSize: wp(5),
  },
  filterImg: {width: wp(6), height: hp(3)},
  detailContainer: {
    marginTop: hp(0.5),
    flexDirection: 'row',
    alignItems: 'center',
  },
  modalView: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
    paddingHorizontal: wp(3),
  },
  bottomView: {
    flex: 1,
  },
  content: {
    justifyContent: 'center',
    alignItems: 'center',
    paddingHorizontal: wp(3),
    paddingBottom: Platform.OS == 'ios' ? hp(10) : hp(1),
  },
  indicatorContainer: {alignSelf: 'center'},
  crossContainer: {
    position: 'absolute',
    top: hp(1),
    right: Platform.OS == 'ios' ? wp(2) : wp(3),
  },
  optionText: {
    color: Colors.greyIcn,
    fontFamily: Typography?.poppins?.Regular,
    fontSize: wp(3),
    marginRight: wp(1.5),
  },
  valueText: {
    color: Colors.black,
    fontFamily: Typography?.poppins?.Medium,
    fontSize: wp(3),
  },
  optionContainer: {
    position: 'absolute',
    zIndex: 1000,
    backgroundColor: 'white',
    paddingHorizontal: wp(2),
    paddingVertical: hp(1),
    borderRadius: wp(2),
    shadowColor: '#000',
    shadowOffset: {width: 0, height: 2},
    shadowOpacity: 0.25,
    shadowRadius: 3.84,
    elevation: 24,
  },
  forwardIcn: {width: wp(3.5), height: hp(2), marginLeft: wp(3)},
  carModel: {
    color: Colors.greyIcn,
    fontFamily: Typography?.poppins?.Regular,
    fontSize: wp(3.5),
  },
  button: {marginTop: hp(3), width: wp(84), alignSelf: 'center'},
  placeholderText: {
    color: Colors.primary,
    fontSize: wp(3.5),
    fontFamily: Typography?.poppins?.Medium,
    marginBottom: hp(0.5),
  },
  filterPlaceholderText: {
    color: Colors.primary,
    fontSize: wp(3.8),
    fontFamily: Typography?.poppins?.Medium,
    marginBottom: hp(0.5),
  },
  blackPlaceholderText: {
    color: Colors.black,
    fontSize: wp(3.5),
    fontFamily: Typography?.poppins?.Medium,
    marginLeft: wp(1),
  },
  filterBlackPlaceholderText: {
    color: Colors.black,
    fontSize: wp(3.8),
    fontFamily: Typography?.poppins?.Medium,
    marginLeft: wp(1),
    marginTop: hp(1),
    marginBottom: hp(0.4),
  },
  submitText: {
    color: Colors.black,
    fontSize: wp(3.8),
    fontFamily: Typography?.poppins?.Medium,
  },
  error: {
    maxWidth: wp(43),
    marginTop: hp(0.4),
    marginLeft: wp(0.8),
    color: 'red',
    fontSize: wp(3.3),
    fontFamily: Typography?.poppins?.Regular,
  },
  filterHeading: {
    fontSize: wp(5),
    marginTop: hp(2),
    color: Colors.black,
    fontFamily: Typography?.poppins?.Medium,
  },
  filterDropDownContainer: {
    width: wp(94),
    alignSelf: 'center',
    marginTop: hp(2),
  },
  filterContainer: {
    paddingHorizontal: wp(5),
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: Colors.dullWhite,
    borderRadius: wp(2),
    marginRight: wp(3),
    paddingVertical: hp(1.7),
  },
  filterIcn: {width: wp(4.4), height: hp(2.5), marginRight: wp(1.4)},
  filterText: {
    fontSize: wp(3.5),
    height: hp(2.5),
    color: Colors.black,
    fontFamily: Typography?.poppins?.Regular,
  },
  primaryInput: {
    width: wp(94),
    borderWidth: wp(0.2),
    borderColor: Colors.primary,
    color: Colors.black,
    paddingVertical: hp(1.2),
    borderRadius: wp(2),
    fontFamily: Typography?.poppins?.Regular,
    backgroundColor: Colors.dullWhite,
    paddingHorizontal: wp(4),
  },
  input: {
    width: wp(94),
    color: Colors.black,
    paddingVertical: hp(1.2),
    borderRadius: wp(2),
    fontFamily: Typography?.poppins?.Regular,
    backgroundColor: Colors.dullWhite,
    paddingHorizontal: wp(4),
  },
  upperIndicator: {
    backgroundColor: Colors.dashBoardInactive,
    paddingHorizontal: wp(5),
    paddingVertical: hp(0.3),
    borderRadius: wp(3),
  },
  lowerIndicator: {
    backgroundColor: Colors.dashBoardInactive,
    paddingHorizontal: wp(5),
    paddingVertical: hp(0.3),
    borderRadius: wp(3),
    marginTop: hp(0.3),
  },
  submitContainer: {
    paddingHorizontal: wp(15),
    paddingVertical: hp(1),
    borderRadius: wp(2),
    borderWidth: wp(0.2),
    borderColor: Colors.primary,
  },
  yesContainer: {
    paddingHorizontal: wp(15),
    paddingVertical: hp(1),
    borderRadius: wp(2),
    borderWidth: wp(0.2),
    backgroundColor: Colors.primary,
    marginLeft: wp(3),
  },
  clearText: {
    width: wp(30),
    paddingVertical: hp(0.9),
    borderRadius: wp(2),
  },
  clearButtonText: {
    color: Colors.white,
    fontSize: wp(3.8),
    fontFamily: Typography?.poppins?.Medium,
  },
  plusIcn: {
    position: 'absolute',
    bottom: hp(5),
    right: wp(3),
    alignItems: 'flex-end',
  },
  plus: {width: wp(17), height: hp(8.5)},
  plusOptionsContainer: {
    backgroundColor: 'white',
    alignItems: 'center',
    paddingHorizontal: wp(3),
    paddingTop: hp(2),
    borderTopLeftRadius: wp(4),
    borderTopRightRadius: wp(4),
    borderBottomLeftRadius: wp(4),
    marginRight: wp(11),
    shadowColor: '#000',
    shadowOffset: {
      width: 0,
      height: 11,
    },
    shadowOpacity: 0.57,
    shadowRadius: 15.19,
    elevation: 24,
  },
  dateContainer: {marginTop: hp(2)},
  applyButton: {marginTop: hp(4)},
  activityIndicator: {
    position: 'absolute',
    alignSelf: 'center',
    top: hp(50),
  },
  noDataAvailable: {
    color: Colors.black,
    fontSize: wp(4),
    alignSelf: 'center',
    fontFamily: Typography?.poppins?.Medium,
  },
  bottomSheetPlaceholderText: {
    color: Colors.black,
    fontSize: wp(3.5),
    fontFamily: Typography?.poppins?.Medium,
    marginLeft:wp(1),
    marginTop:hp(2),
    marginBottom:hp(0.4)
  },
  filterBottomDropDownContainer: {
    width: wp(94),
    alignSelf: 'center',
  },
});
